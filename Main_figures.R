...
p1=ggplot(d2t%>%
  ...
  geom_hline(data=tibble(Scena="Local facilitation",Delta="Local dispersal",y=c(0,.4)),aes(yintercept=y))+
  geom_text(data= tibble(Scena="Local facilitation",Delta="Local dispersal",y=c(0.015,.415),x=1.05,labels=c("b","c")), 
...
y_line_color=c(0.48,.14,.85,.22)
for (i in 1:4){
  if (i%%2 ==0){
    
    assign(paste0("p2_",i),
           ggplot(d2%>%melt(., measure.vars=c("CSI"))%>%
                    filter(., Scena=="global_C_local_F",Delta==.1)%>%filter(., alpha_0 %in% c(0,.4)[abs(i/2)])%>%
             ...
             geom_point(data=d2t%>%
                          ...
                          filter(., Scena=="Local facilitation",Delta==.1,alpha_0==c(0,.4)[abs(i/2)])%>%
    ...
    assign(paste0("p2_",i),
           ggplot(d2%>%melt(., measure.vars=c("Stress_tolerant","Competitive"))%>%
                    filter(., Scena=="global_C_local_F",Delta==.1)%>%filter(., alpha_0 %in% c(0,0,.4)[i])%>%
                    mutate(.,variable=recode_factor(variable,"Stress_tolerant"="Stress-tolerant")))+
             
             geom_path(aes(x=Stress,y=value,linetype=Branches,color=variable,group=interaction(Branches,variable)))+
             facet_grid(.~alpha_0,labeller=label_bquote(cols=alpha[e]==.(alpha_0)))+
             the_theme+theme(strip.text.x = element_blank(),strip.background.x = element_blank(),legend.text = element_text(size=12))+
             ...
             geom_point(data=d2t%>%
                          mutate(all_state=recode_factor(all_state,
                                                         ...
                                                         "Stress_tolerant/Coexistence"="Coexistence/Stress_tolerant",
                                                         "Desert/Stress_tolerant"="Stress_tolerant/Desert"))%>%
                          filter(., Scena=="Local facilitation",Delta==.1,alpha_0==c(0,0,.4)[i])%>%
                          mutate(., y=y_line_color[i]),
                        aes(x=Stress,y=y,fill=all_state,color=all_state),alpha=.5,shape=22,size=3)+
             scale_fill_manual(values=c("Coexistence" = "#D8CC7B", 
                                        "Competitive" = "#ACD87B", 
...
p_bottom=ggarrange(ggplot()+theme_void(), p2_1+ylab("Cover")+guides(color="none",fill="none"),
                   p2_3+ylab("Cover")+guides(color="none",fill="none"),ggplot()+theme_void(),
                   common.legend = T,legend = "bottom",ncol = 4,align="hv",labels=c("",letters[2],letters[3],""),widths = c(.3,1,1,.3))

Fig_2=ggarrange(p1,p_bottom,nrow=2,heights = c(3,1.3),labels = c(letters[1],""))

ggsave("../Figures/Final_figs/Figure_2.pdf",Fig_2,width = 9,height = 10)


Fig_2=ggarrange(p1_pattern+guides(pattern=F),p_bottom,nrow=2,heights = c(3,1.3),labels = c(letters[1],""))

ggsave("../Figures/Final_figs/Figure_2_pattern.pdf",Fig_2,width = 9,height = 10)


# Figure 3: Mulitstability trait ----
    ...
    for (a0 in unique(d_state$alpha_0)){
      ...
      if (i>j){
        d_final=rbind(d_final,tibble(Psi1=i,Psi2=j,alpha_0=a0,
                                     ...
                                     Frac_bistab_env=length(which(d_fil$all_state %in% c("Stress_tolerant/Desert")))/length(which(d_fil$state!="Desert")),
                                     Frac_bistab_env_raw=length(which(d_fil$all_state %in% c("Stress_tolerant/Desert"))),
...
p1=ggplot(d_final)+
  ...
  the_theme+labs(x=TeX(r'(Species 1 trait, $\psi_1$)'),y=TeX(r'(Species 2 trait, $\psi_2$)'),fill="Fraction of community bistability \n along the stress gradient")+
  scale_fill_gradientn(colors=colorRampPalette(c("#E6F1DC","#D4F7B3","#9AD660","#42790D"))(100))+
  ...
  geom_text(data=tibble(x=c(trait1_for_bifu[1],trait1_for_bifu[2]+.05,trait1_for_bifu[3]+.05),
                        ...
                        txt=c("(i)","(ii)","(iii)")),aes(x=x,y=y,label=txt),size=4.5)+
  guides(shape=F)

p2=ggplot(d_final)+
  geom_tile(aes(x=Psi1,y=Psi2,fill=Deg-Rest))+
  the_theme+labs(x=TeX(r'(Species 1 trait, $\psi_1$)'),y=TeX(r'(Species 2 trait, $\psi_2$)'),fill="Species 2 hysteresis size")+
...
d_final$comp_expe=0.3*(1+(d_final$Psi1)*exp(-abs(d_final$Psi1-d_final$Psi2)))
ggplot(d_final)+
  geom_tile(aes(x=Psi1,y=Psi2,fill=comp_expe))+
  the_theme+labs(x=TeX(r'(Species 1 trait, $\psi_1$)'),y=TeX(r'(Species 2 trait, $\psi_2$)'),fill="Species 2 hysteresis size")+
  scale_fill_gradientn(colors=colorRampPalette(c("#FFFFFF","#E2BFE2","#D48ACF","#650328"))(100))+
  guides(shape=F)
  ...
  if (i %in% c(1,2)){
    assign(paste0("p3_",i),
           ggplot(d2%>%
                    ...
                    mutate(., variable=recode_factor(variable,"Stress_tolerant"="Sp1","Competitive"="Sp2")))+
...
pbifu=ggarrange(ggplot()+theme_void(),p3_1,p3_2,
                ggarrange(p3_3+theme(legend.position = "none"),get_legend(p3_32+guides(color=F)),
                          get_legend(p3_32+guides(linetype=F)),nrow = 3,heights = c(1,.2,.2)),
                ggplot()+theme_void(),nrow=5,
                labels = c("","(i)","(ii)","(iii)",""),heights = c(.3,1,1,1.7,.3),hjust = .5)
p_tot=ggarrange(ggarrange(p1,p2,p3,p4,ncol = 2,nrow=2,labels = letters[1:4]),ggplot()+theme_void(),
                pbifu,ncol=3,widths = c(2,.05,.8))

ggsave("../Figures/Final_figs/Figure_3",p_tot,width = 13,height = 9)
