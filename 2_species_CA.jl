...
function Gillespie_tau_leeping(; landscape, param, time, type_competition, save=false, burning=1500, N_snap=25, name_save="")
    ...
    if type_competition == "global" #competition occurs globally
        ...
        for t in 1:time
            ...
            Rate_landscape[:, :, 1] .= @. (delta * rho_1 + (1 - delta) * neigh_1 / z) * @.(beta * (1 - S * (1 - psi1 * e)) - ((cintra * rho_1 + alpha_0 * h * (1 + psi1 * exp(-abs(psi1 - psi2))) * rho_2)))
            Rate_landscape[:, :, 1] .= Rate_landscape[:, :, 1] .* (landscape .== 0)

            Rate_landscape[:, :, 2] .= @. (delta * rho_2 + (1 - delta) * neigh_2 / z) * @.(beta * (1 - S) - ((cintra * rho_2 + alpha_0 * (1 + psi1 * exp(-abs(psi1 - psi2))) * rho_1)))
            ...
            Rate_landscape[:, :, 5] .= @.(r + f * (psi1 * (neigh_1 / z) + psi2 * (neigh_2 / z))) .* (landscape .== -1)
        ...
        for t in 1:time
            ...
            Rate_landscape[:, :, 1] .= @. (delta * rho_1 + (1 - delta) * neigh_1 / z) * @.(beta * (1 - S * (1 - psi1 * e)) - ((cintra * (neigh_1 / z) + alpha_0 * h * (1 + psi1 * exp(-abs(psi1 - psi2))) * (neigh_2 / z))))
            Rate_landscape[:, :, 1] .= Rate_landscape[:, :, 1] .* (landscape .== 0)

            Rate_landscape[:, :, 2] .= @. (delta * rho_2 + (1 - delta) * neigh_2 / z) * @.(beta * (1 - S) - ((cintra * (neigh_2 / z) + alpha_0 * (1 + psi1 * exp(-abs(psi1 - psi2))) * (neigh_1 / z))))
            Rate_landscape[:, :, 2] .= Rate_landscape[:, :, 2] .* (landscape .== 0)
            ...
            for stress in S_seq
                ...
                for alpha_e in c_seq
                    ...
                    for rep in 1:replicate
                        ...
                        name_landscape = "../Table/CA/Clustering_species/" * repr(disp) * "/Landscape_Scalecomp_" * scale_comp * "_Stress_" * repr(stress) * "_alpha0_" * repr(alpha_e) * "_cintra_" * repr(aii)
...
for stress in S_seq
    for nrep in 1:3
        ...
        CSV.write("../Table/Patch_size/Example_sim/Landscape_size_100_stress_" * repr(stress) * "_" * repr(nrep) * ".csv", Tables.table(state), writeheader=false)
    end
end
    ...
    for scena in (2)
        ...
        for stress in S_seq
            for nrep in 1:3
                ...
                CSV.write("../Table/Patch_size/Competition_regime/Landscape_size_100_stress_" * repr(stress) * "_" * name_scena * "_" * repr(nrep) * "_relat_compet_" * repr(alpha0) * ".csv", Tables.table(state), writeheader=false)
            end
        end
    end
end
...
for scena in (1)
    for stress in S_seq_prime
        ...
        for alpha0 in [0, 0.15, 0.3]
            ...
            name_landscape = "../Table/Patch_size/Big_sim/Landscape_" * name_scena * "_S_" * repr(round(stress, digits=2)) * "_alpha0_" * repr(alpha0)
